...
export function createAdminService(config: AdminServiceConfig) {
	...
	async function batchTranslate(request: BatchTranslateRequest): Promise<BatchTranslateResponse> {
		...
		// Build map for saving
		const sourceTextMap = new Map(items.map((item) => [item.sourceHash, item.sourceText]));

		// Save translations
		let savedCount = 0;
		for (const t of translations) {
			try {
				const sourceText = sourceTextMap.get(t.sourceHash);
