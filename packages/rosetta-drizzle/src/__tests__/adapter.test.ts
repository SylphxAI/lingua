...
describe('DrizzleStorageAdapter', () => {
	...
	describe('registerSources', () => {
		test('inserts new sources', async () => {
			const { adapter } = createAdapter();

			await adapter.registerSources([
				{ hash: 'hash1', text: 'Hello' },
				{ hash: 'hash2', text: 'World', context: 'greeting' },
			]);

			const sources = await adapter.getSources();
			expect(sources.length).toBe(2);
			expect(sources[0]!.hash).toBe('hash1');
			expect(sources[0]!.text).toBe('Hello');
			expect(sources[1]!.context).toBe('greeting');
		});

		test('does not insert duplicates', async () => {
			const { adapter } = createAdapter();

			await adapter.registerSources([{ hash: 'hash1', text: 'Hello' }]);
			await adapter.registerSources([{ hash: 'hash1', text: 'Hello' }]);
		...
		test('increments occurrences for existing sources', async () => {
			const { adapter } = createAdapter();
			...
			const sources = await adapter.getSources();
			expect(sources[0]!.occurrences).toBe(3);
		});
		...
		test('handles batch with mixed new and existing', async () => {
			const { adapter } = createAdapter();
			...
			const hash1Source = sources.find((s) => s.hash === 'hash1');
			expect(hash1Source!.occurrences).toBe(2);
		});
	});
	...
	describe('saveTranslation', () => {
		...
		test('sets autoGenerated flag', async () => {
			...
			const withStatus = await adapter.getSourcesWithTranslations(['zh-TW']);
			// Need to register source first
			await adapter.registerSources([{ hash: 'hash1', text: 'Hello' }]);
			const result = await adapter.getSourcesWithTranslations(['zh-TW']);
			expect(result[0]!.translations['zh-TW']?.autoGenerated).toBe(true);
		});

		test('registers source if sourceText provided', async () => {
			const { adapter } = createAdapter();
			...
			expect(sources.length).toBe(1);
			expect(sources[0]!.text).toBe('Hello');
			expect(sources[0]!.context).toBe('greeting');
		});
	});
...
describe('DrizzleStorageAdapter Admin Methods', () => {
	describe('getSourcesWithTranslations', () => {
		test('returns sources with translation status', async () => {
			...
			expect(source1!.translations['zh-TW']).toEqual({
				text: '你好',
				autoGenerated: true,
				reviewed: false,
			});
	...
	describe('saveTranslations (batch)', () => {
		...
		test('registers source texts if provided', async () => {
			const { adapter } = createAdapter();

			await adapter.saveTranslations('zh-TW', [
				{ hash: 'hash1', text: '你好', sourceText: 'Hello' },
				{ hash: 'hash2', text: '世界', sourceText: 'World' },
			]);
...
describe('DrizzleStorageAdapter Edge Cases', () => {
	...
	test('handles very long text', async () => {
		...
		await adapter.registerSources([{ hash: 'long', text: longText }]);
	...
	test('concurrent operations do not corrupt data', async () => {
		const { adapter } = createAdapter();

		// Simulate concurrent registrations
		const promises = Array.from({ length: 100 }, (_, i) =>
			adapter.registerSources([{ hash: `hash${i}`, text: `Text ${i}` }])
		);
